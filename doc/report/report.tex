...
Computation time of the both algorithm is in the same order on the small dataset we used, where $n_M = 51$ and $n_U = 892$. However, the expected computation time on the full dataset of 500K users and 17K movies is \TODO{XXXXXXX} for mixture virsus 2 hours for low-rank approximation. \TODO{comp time is proportional to $n_U \cdot n_M$ in mixture while it is proportional to $(n_U + n_M)$ in low-rank approximation. brabrabra}. This is another reason why most of the leading teams in the Netflix Prize competition does not appear to use the mixture model.
