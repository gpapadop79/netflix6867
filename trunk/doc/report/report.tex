...
got this small fully packed rating matrix. Our subset has 51 movies
and 892 users. When dividing this up into training and test data, we
ended up with a large number of training and testing data.
...
justify the model, then derive the E-M algorithm. We briefly describe
the implementation and analyze the complexity. Finally, we evaluate
its performance by using TODO
...
TODO describe the following better:
This model works well for collaborative filtering, but it is not that
useful for predicting the . Such a task would be a difficult one,
anyway, since we would not have any information about the new users,
and would only be able to go on aggregate distributions.
...
We implemented the E-M algorithm in MATLAB. The performance was
poor when run on the dataset.

\subsection{Complexity Analysis}

TODO the complexity

The number of parameters in our system is
\begin{align}
(k_R - 1) \cdot k_U \cdot k_M +
(k_U - 1) \cdot n_U +
(k_M - 1) \cdot n_M
\end{align}
(A multinomial of $k$ possible outcomes needs only $k-1$ parameters.)
In the M-step, we need to update this number of parameters.
...
there will be no motivation to go with anything but the largest number
of clusters.  the number of user types and number of movie types will
become equal to the number of cell types. , and then decide to .

One common and easy-to-use metric is the {\em Bayesian information
  criterion} (BIC), also known as the {\em Schwarz information
